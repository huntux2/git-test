	...
	<head>
   		<script type="text/javascript">
   			...
   			function doGridInit(){
				...
				$("#gridBody").jqGrid({
					datatype: "LOCAL",
	   				...
	   				rowNum : 'max',
	   				pager: '#footer',
	   				colNames:["수정", "번호", "지역", "역 명", "등록자", "등록일", "수정자", "수정일"],
	          		colModel : [
			...
			function findStatnList(mode){
				/*검색조건*/
	   			var area = $.trim($("#areaSelect").val());	  /*지역*/
	   			var statnNm = $.trim($("#statnNmText").val()); /*역 명*/
   				...
   				$("#addDialog").dialog({
					...
					buttons: {
						"등록": function() {
							...
							else if(confirm("역을 등록하시겠습니까?")){
								/*역 명 형식*/
								var form = /(.+[역\b])/gi;
								if(!form.test(statnNm.val())){
									statnNm.val(statnNm.val()+"역");
								}
								
								$.ajax({
									type:"POST",
									url: "/admin/statnProcess.do",
									data:{state:"insert", areaCode:area.val(), statnNm:statnNm.val(), register:"${name}"},
									Type:"JSON",
									success : function(data) {
										if(data.errorCode == 0){
											alert("등록이 완료되었습니다.");
											...
											/* 등록한 지역으로 자동 검색 */
											$("#areaSelect option[value='"+area.val()+"']").attr("selected", "selected");
											findStatnList("area");
										}else{
											alert("등록실패");
										}
						...
						"수정": function() {
							...
							/*conForm 생성(확인 : 수정, 취소 : 상태유지)*/
								...
								$.ajax({
									type:"POST",
									url: "/admin/statnProcess.do",
									data:{state:"update", statnCode:statnCode, areaCode:area.val(), statnNm:statnNm.val(), updUsr:"${name}"},
									Type:"JSON",
									success : function(data) {
										if(data.errorCode == 0){
											alert("수정이 완료되었습니다.");
								...
								if(data.errorCode == 0){
									...
									for(var i = 0; i < rowIds.length; i++){
										if($("#gridBody").getRowData(rowIds[i]).statnCode == data.errorMsg){
											msgArray.push($("#gridBody").getRowData(rowIds[i]).no+": "+$("#gridBody").getRowData(rowIds[i]).statnNm);
										}
									}
									
									alert(msgArray+"은 현재 사용중인 역 입니다.");
									findStatnList("area");
								}
							}, /*success end*/
							error : function(request, status, error){
   	...
   	<body>
   		...
   		<!-- 사용방법 -->
   		<div class="caption">
			<div>* 지역을 선택한 후 조회버튼을 이용하여 역정보를 조회할 수 있습니다.</div>
			<div>* 역명을 입력한 후 조회버튼을 이용하여 역정보를 조회할 수 있습니다.</div>
			<div>* 등록버튼을 이용하여 역정보를 등록할 수 있습니다.</div>
			<div>* 삭제할 역정보를 체크박스로 선택한 후 석제버튼을 이용하여 역정보를 삭제할 수 있습니다.</div>
			<div>* 수정버튼을 이용하여 역정보를 수정할 수 있습니다.</div>
   		</div>
