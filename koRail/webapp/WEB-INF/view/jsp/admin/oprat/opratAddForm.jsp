...
<html>
	<head>
		<script type="text/javascript">
   			var viewState = false;		 		/*viewState와 같은 역활이며 경유지에 대한 상태이다*/
   			var viewState2 = false;				/*viewState와 같은 역활이며 호실정보에 대한 상태이다*/
   			var detailOpratArray = new Array();	/* 경유지 */
   			var roomArray = new Array();		/*호실*/
   			...
   			*************************************************/
   			...
   			function setYear(){
				for(var i = 1; i < 3; i++){
	   				$("#year"+i).html("");
   	   				$("#year"+i).append('<option value="'+(new Date().getFullYear()-1)+'">'+(new Date().getFullYear()-1)+'</option>');
   	   				$("#year"+i).append('<option value="'+new Date().getFullYear()+'">'+new Date().getFullYear()+'</option>');
   	   				$("#year"+i).append('<option value="'+(new Date().getFullYear()+1)+'">'+(new Date().getFullYear()+1)+'</option>');   				
   			...
   			function setSelectd(){
				...
				/* 현재날짜 자동선택 */
				for(var i = 1; i < 3; i++){
					$("#year"+i).children("option[value='"+new Date().getFullYear()+"']").attr("selected", "selected");
					
					if(month < 10){
	  	   				$("#month"+i).children("option[value='0"+month+"']").attr("selected", "selected");
					}else{
						$("#month"+i).children("option[value='"+month+"']").attr("selected", "selected");
					}
					
					if(hh24 < 10){
						$("#hh24"+i).children("option[value='0"+hh24+"']").attr("selected", "selected");
					}else{
						$("#hh24"+i).children("option[value='"+hh24+"']").attr("selected", "selected");
					}
					
					if(mi < 10){
						$("#mi"+i).children("option[value='0"+mi+"']").attr("selected", "selected");
					}else{
						$("#mi"+i).children("option[value='"+mi+"']").attr("selected", "selected");	
					}
					
					if(dd < 10){
	  					$("#date"+i).children("option[value='0"+dd+"']").attr("selected", "selected");
	  				}else{
	  					$("#date"+i).children("option[value='"+dd+"']").attr("selected", "selected");
	  				}
				}
   			...
   			function setDetailOpratDialog(){
				...
				/*도착일시*/
   				...
   				if($("#startStatnCode").val() == "" || $("#startStatnCode").val() == null){
					...
					/* 이전역 검색 번튼 */
					if($("#gridBody").getRowData(0).state != "insert"){
   			...
   			**********************************************/
	   		...
	   		function addOprat(){
				...
				if($("#gridBody2").getRowData(0).state == "non"){
					...
					var array1 = $("#gridBody").getRowData();  /* 경유지 */
   	...
   	<body>
   		...
   		<div class="caption">
			...
			<div>* 출발일시와 도착일시는 기본적으로 현재 날짜 및 시각이 자동선택되어있습니다.</div>
			...
			<div>* 모든 작업이 완료되었다면 등록버튼을 이용하여 입력한 정보를 등록 할 수 있습니다.</div>
				...
				<table class="d-table">
					...
					<tbody>
						<tr>
							...
							<td>
								<input id="trainNo" type="text" style="width: 76%;" disabled="disabled">
								<button type="button" onclick="setTrainDialog();" style="width: 20%;">검색</button>
							</td>
							<td class="head">열차종류</td>
							<td colspan="2">
								<input id="trainKnd" style="width: 98%;" type="text" disabled="disabled">
							</td>
						</tr>
						<tr>
							...
							<td>
								...
								<input id="startStatnValue" style="width: 76%;" type="text" disabled="disabled">
						...
						<tr>
							...
							<td>
								...
								<input id="arvlStatnValue" style="width: 76%;" type="text" disabled="disabled">
						...
						<tr>
							...
							<td>
								...
								<input id="routeValue" type="text" disabled="disabled" value="경부선" style="width: 76%;">
								<button type="button" style="width: 20%;">검색</button>
							</td>
							<td class="head">요금</td>
							<td>
								<input name="fare" type="text" onkeydown="doNumberCheck(this, event, 13, toCommaNumber(this));" onkeyup="toCommaNumber(this);" style="width: 98%;" dir="rtl">
							</td>
				...
				<tbody>
					<tr>
						...
						<td style="border: 1px solid #FFFFFF;">
							<input id="dTrainNo" type="text" onkeydown="doNumberCheck(this, event, 20)" style="height: 26px;">
						</td>
						<td style="border: 1px solid #FFFFFF;">
